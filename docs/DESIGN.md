...
Status: agreed 2026-07-13. Supersedes the "Bookwalk" research memo.
A working prototype of §3 and §4 exists in this repo.
...
Every waypoint carries a **`quote_anchor`**: a verbatim string that must appear
**exactly once** in `source.txt`. The Zod schema (§7.3) finds it and derives:

```
position = offset(quote_anchor) / len(source.txt)     # 0.0 … 1.0
```
...
- `book.ordering_key` — `"clock" | "chapter" | "position"`. Presentation only:
...
Only `simultaneous_with: [<id>, …]` — mutual, and validated as mutual — earns
...
```jsonc
"place_certainty": "explicit" | "inferred" | "inspired_by" | "disputed",
"certainty_note": "required unless explicit — why you placed it there"
```
...
`tools/plates.py` from the same tile sources the live reader uses. A retina
...
So `tools/extract.py` mines `source.txt` and produces a **worklist**, never a
waypoint:
...
| `place_certainty` | **human** | explicit / inferred / inspired_by / disputed |
| `certainty_note` | **human** | why you placed it there |
...
- **Per-book maintainers.** Whoever brings a book owns its queue (Debian's
  ...
  on one person.
- **Ship per-waypoint, not per-book.** `editorial_status: draft | reviewed |
...
export const waypoint = z.object({ … }).superRefine((wp, ctx) => {
  const hits = occurrences(sourceText(wp.book), normalise(wp.quote_anchor));
...
in their editor from the emitted JSON Schema. This replaces `tools/validate.py`;
...
§3.2 replaces it with `position` + `progress_label` + `book.ordering_key`.
